...
class Test_Reader < Test::Unit::TestCase
  ...
  def test_validation_4
    self.assert_raise(FormatError){Reader.new(filename: TEST_INPUT_BAD_FILE_NAME_4)}
  end

  def test_validation_5
    self.assert_raise(FormatError){Reader.new(filename: TEST_INPUT_BAD_FILE_NAME_5)}
  end

  def test_validation_6
    self.assert_raise(FormatError){Reader.new(filename: TEST_INPUT_BAD_FILE_NAME_6)}
  end

  def test_validation_7
    self.assert_raise(FormatError){Reader.new(filename: TEST_INPUT_BAD_FILE_NAME_7)}
  end
  ...
  def test_validation_9
    self.assert_raise(FormatError){Reader.new(filename: TEST_INPUT_BAD_FILE_NAME_9)}
  end

  def test_validation_10
    exception = self.assert_raise(FormatError){Reader.new(filename: TEST_INPUT_BAD_FILE_NAME_10)}
    self.assert_equal("Format error on line# 1: Operation count must be a non-negative integer", exception.message)
  end
